...
struct Fifo
{
    void prepare(int numChannels, int numSamples)
    {
        static_assert(std::is_same_v<T, juce::AudioBuffer<float>>,
                      "prepare(int numChannels, int numSamples) should only be used when Fifo is holding juce::AudioBuffer<float>");
...
{
    SingleChannelSampleFifo(Channel ch) : channel(ch)
    {
        prepared.set(false);
    }
    ...
    void update(const BlockType& buffer)
    {
        ...
        {
            pushSampleIntoFifo(channelPtr[i]);
        }
    }
